...
tw = [tuple(a) for a in zip([0, 0, 0, 7, 7, 14, 14, 14], [0, 7, 14, 0, 14, 0, 7, 14])]
dw = [tuple(a) for a in zip([1, 1, 2, 2, 3, 3, 4, 4, 7, 10, 10, 11, 11, 12, 12, 13, 13],
                           [1, 13, 2, 12, 3, 11, 4, 10, 7, 4, 10, 3, 11, 2, 12, 1, 13])]
tl = [tuple(a) for a in zip([1, 1, 5, 5, 5, 5, 9, 9, 9, 9, 13, 13], [5, 9, 1, 5, 9, 13, 1, 5, 9, 13, 5, 9])]
dl = [tuple(a) for a in zip([0, 0, 2, 2, 3, 3, 3, 6, 6, 6, 6, 7, 7, 8, 8, 8, 8, 11, 11, 11, 12, 12, 13, 13],
                           [3, 11, 6, 8, 0, 7, 14, 2, 6, 8, 12, 3, 11, 2, 6, 8, 12, 0, 7, 14, 6, 8, 3, 11])]


def scorer(move, wordsposition, board):
    letters = move[0]
    positions = list(map(tuple, move[1]))  # converting it into tuples which are hashable for the dict
...
def endscore(players, turn):  # to score when the game ends (one player empties his/her pouch)
